...
├── app/                    # Next.js 前端页面（App Router），含 Marketing 路由组（Hero / Stats / Contact）和 Chat 页面
├── components/             # React 组件库，包含按钮、输入框、聊天气泡等可复用的 UI 组件
├── lib/                    # 前端工具函数（cn 类名合并、SEO 元数据生成、品牌常量等）
├── data/                   # 静态数据（项目卡片 achievement-stats、Chat 快捷问题 suggested-actions）
...
- [01-backend-walkthrough-CN.md](./01-backend-walkthrough-CN.md) — 后端代码详解（FastAPI + Bedrock）
- [02-frontend-walkthrough-CN.md](./02-frontend-walkthrough-CN.md) — 前端代码详解（Next.js + AI SDK + Neural Fabric 设计系统）
